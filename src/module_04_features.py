...
def assemble_feature_matrix(df: pd.DataFrame, logger) -> tuple[pd.DataFrame, list[str], dict]:
    ...
    exclude_patterns = [
        ...
        "concentration_mg_per_ml",  # raw — replaced by engineered version
        # Target-pair flags are metadata for applicability-domain checks and
        # leave-one-species-out CV (Modules 6/11) — NOT model inputs. Including
        # them as features would let the model directly learn "is this the
        # target pair" as a shortcut, which defeats the point of testing
        # generalisation.
...
def run(config: dict, debug: bool = False) -> pd.DataFrame:
    ...
    with ModuleRunner(
        ...
        # Also save the full df with targets for downstream use. Includes
        # donor_is_target_species / receiver_is_target_species as metadata
        # (not features) so Module 11's applicability domain check can report
        # specifically on Lantana camara → Zea mays rows.
