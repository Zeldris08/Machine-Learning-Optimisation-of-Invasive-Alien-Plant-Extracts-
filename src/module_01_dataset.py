...
Strategy for this practice round:
  1. Rescue concentration values buried in the notes column (regex extraction)
  2. Rescue plant_species / crop_species from context where obviously mislabelled
  3. Keep every row that has at least one endpoint OR a rescueable concentration —
     synthesise whatever is still missing using biologically-grounded models
  4. Generate a synthetic training corpus large enough to learn from, anchored
     to the real dose-response biology of plant allelopathy extracts

Synthetic biology rules (defensible at ISEF level):
  - Germination follows a sigmoidal inhibition curve from ~90% (control) to
    ~5% at high concentration, with species-level noise
  - Root length: 60–120 mm at control, proportionally suppressed
  - Shoot length: 80–150 mm at control, proportionally suppressed
  - Low concentrations (< 5 mg/mL) sometimes show hormetic stimulation (+5–15%)
  - All values clipped to biologically valid ranges before saving

Usage:
    python src/module_01_dataset.py [--debug]
"""
...
def run(config: dict, debug: bool = False) -> pd.DataFrame:
    ...
    with ModuleRunner(
        ...
        # --- Combine ---
        # COMMENT OUT the concat line below and UNCOMMENT 'combined = real_df' to stop generating synthetic data
