...
POST-EXPERIMENT MODULE. Run only after wet lab data has been collected.

Expected CSV schema (data/experimental/experimental_results.csv):

    concentration_mg_per_ml      — required, all 6 concentrations
    root_length_mm_r1/r2/r3      — required
    shoot_length_mm_r1/r2/r3     — required
    tpc_mg_gae_per_g_r1/r2/r3    — optional; only fill for 0, 0.5, 1, 2.5 mg/mL
                                    leave blank for 5 and 25 mg/mL rows

Notes:
  - germination_percent has no wet-lab replicates in this study.
    The model's germination predictions still appear in the comparison
    table, but with observed_mean = NaN and no metrics computed.
  - TPC is a corroborating chemistry measurement, not a modelled endpoint.
    It is saved separately to outputs/validation/tpc_observed.json.

Inputs:
  - data/experimental/experimental_results.csv
  - outputs/locked/locked_predictions_[HASH].json
  - outputs/locked/data_manifest.json

Usage:
    python src/module_11_validation.py [--debug]
"""
